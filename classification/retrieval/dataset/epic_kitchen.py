...
class EPICKitchen(data.Dataset):
    def __init__(self, dataset='epic-kitchens-100', list_file=pd.read_pickle('EPIC_val.pkl'),
                 new_length={'RGB': 1, 'Flow': 1, 'Spec': 1, 'IMU': 1}, modality= ['Spec', 'RGB', 'Flow'], image_tmpl={'RGB': 'frame_{:010d}.jpg', 'Flow': 'frame_{:010d}.jpg'},  visual_path='../EPIC-KITCHENS', audio_path=None,
                 imu_path='../EPIC-KITCHENS', resampling_rate=24000, num_segments=1, transform=None,
                 mode='test', use_audio_dict=True, narration_embedding=None, compressed=None):
        ...
        self.narration_embedding = None if narration_embedding is None else np.load(narration_embedding)
        if compressed is not None:
            compressed = np.load(compressed)
            self.compressed = {}
            self.compressed['feature'] = compressed['feature']
            self.compressed['token'] = compressed['token']
        else:
        ...
        if 'IMU' in self.modality:
            if not os.path.exists(self.imu_path + '/acc.pkl'):
                ...
                import pickle
                with open(self.imu_path + '/acc.pkl', 'wb') as f:
                    pickle.dump(self.acc_dict, f)
                with open(self.imu_path + '/gyro.pkl', 'wb') as f:
                    pickle.dump(self.gyro_dict, f)
            else:
                self.acc_dict = pd.read_pickle(self.imu_path + '/acc.pkl')
                self.gyro_dict = pd.read_pickle(self.imu_path + '/gyro.pkl')
    ...
    def _get_imu_data(self, record, idx):
        ...
        #print(record.start_frame, record.end_frame)
        center_frame = int(np.floor((record.start_frame * rgb2imu_fps_ratio))) + idx
        # print(center_frame)
        left_frame = center_frame - 96
        right_frame = center_frame + 96
        ...
        pad = max(0, 192 - acc_data.shape[0])
        acc_data = np.pad(acc_data, ((pad//2, pad - pad//2), (0, 0)), 'constant', constant_values=0)
        pad = max(0, 192 - gyro_data.shape[0])
        gyro_data = np.pad(gyro_data, ((pad//2, pad - pad//2), (0, 0)), 'constant', constant_values=0)
    ...
    def __getitem__(self, index):
        ...
        for m in self.modality:
            ...
            input[m] = img
        if self.narration_embedding is not None:
            narration = self.narration_embedding[index]
            label['feature'] = narration
        if self.compressed is not None:
            data = self.compressed['feature'][index]
            token = self.compressed['token'][index]
            label['compressed_feature'] = (data, token)
        return [input[m] for m in self.modality] + [label]
