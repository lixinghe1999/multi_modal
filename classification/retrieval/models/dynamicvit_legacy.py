        ...
        for i, (blk_a, blk_i) in enumerate(zip(self.audio.blocks, self.image.blocks)):
            if i in self.pruning_loc:
                ...
                if self.training:
                    ...
                    zero_decision = torch.zeros_like(prev_decision)
                    keep_policy = keep_policy.unsqueeze(-1)
                    prev_decision = torch.scatter(zero_decision, 1, keep_policy, torch.ones_like(keep_policy, dtype=zero_decision.dtype))
                p_count += 1
            else:
                if self.training:
