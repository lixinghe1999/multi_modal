...
class AdaConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        stem = nn.Sequential(
            nn.Conv2d(in_chans, dims[0], kernel_size=4, stride=4 if down_sample else 1),
    ...
    def get_costum_param_groups(self, weight_decay):
        ...
        return [
            ...
            {'params': decay, 'weight_decay': weight_decay, 'name': 'base_decay', 'small_lr_scalar': 0.01,
             'fix_step': 5}
        ]
...
class ConvNeXt(nn.Module):
    ...
    def forward_features(self, x):
        for i in range(4):
            x = self.downsample_layers[i](x)
            x = self.stages[i](x)
        x = nn.functional.adaptive_max_pool2d(x, 1)
